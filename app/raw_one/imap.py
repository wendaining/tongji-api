...
def fetch_latest_code(config: ImapConfig) -> str | None:
    ...
    try:
        ...
        code = _search_and_extract(mailbox, '(UNSEEN SUBJECT "{}")'.format(SEARCH_SUBJECT))
        if code:
            return code
        code = _search_and_extract(mailbox, '(SUBJECT "{}")'.format(SEARCH_SUBJECT))
        return code
    finally:
        mailbox.logout()
...
def wait_for_code(
    ...
    *,
    ...
    try:
        ...
        criteria = '(SUBJECT "{}")'.format(SEARCH_SUBJECT).encode("utf-8")
        result, data = mailbox.search(None, criteria)
        if data and data[0]:
            last_seen_id = data[0].split()[-1]
...
def _search_and_extract(mailbox: imaplib.IMAP4_SSL, criteria: str) -> str | None:
    result, data = mailbox.search(None, criteria.encode("utf-8"))
    if not data or not data[0]:
        return None
    latest_id = data[0].split()[-1]
...
def _extract_code_from_email(mailbox: imaplib.IMAP4_SSL, email_id: bytes) -> str | None:
